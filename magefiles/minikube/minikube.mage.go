...
func (Minikube) Init() error {
	...
	if err := sh.Run("docker", "pull", "hub.docker.io/DelineaXPM/dsv-k8s:latest"); err != nil {
		dspin.WarningPrinter.Printfln("docker pull: %v", err)
		return fmt.Errorf("docker pull: %w", err)
	}
	dspin.SuccessPrinter.Println("docker image for hub.docker.io/DelineaXPM/dsv-k8s:latest pulled")
