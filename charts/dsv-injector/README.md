...
| recreateSelfSignedCertThreshold | int | 90 | recreateSelfSignedCertThreshold specifies the number of days before expiration to recreate the self-signed certificate When an existing certificate's age exceeds (webhookCertExpireDays - recreateSelfSignedCertThreshold), it will be regenerated. For example, with webhookCertExpireDays=365 and recreateSelfSignedCertThreshold=90, certs will be regenerated after 275 days. |
