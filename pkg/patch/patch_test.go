...
//
// - Create a secret with the following command to test against: `dsv secret create --path 'tests:dsv-k8s' --data '{"password": "","username": ""}'`
// - Hard delete to reset test: `dsv secret delete --path 'tests:dsv-k8s' --force`
// - Rollback to a prior version: `dsv secret rollback --path 'tests:dsv-k8s' --version 0`.
package patch
